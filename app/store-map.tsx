...
export default function StoreMap(){
  ...
  const filtered=useMemo(()=>{
    const needle=normalize(query),result=stores.filter(store=>{const availability=availabilityFor(store);return(!needle||normalize(`${store.name}${store.address}`).includes(needle))&&(municipality==="すべての地域"||store.municipality===municipality)&&(availabilityFilter==="all"||availabilityFilter==="unknown"&&availability===null||availability===availabilityFilter)&&(!favoritesOnly||favorites.includes(store.id))});
  ...
  useEffect(()=>{const L=leafletRef.current?.default,layer=storeLayer.current;if(!L||!layer)return;layer.clearLayers();storeGroups.forEach(group=>{const first=group[0],isMultiple=group.length>1,isSelected=group.some(store=>store.id===selectedId),statuses=new Set(group.map(availabilityFor)),markerStatus=statuses.size===1?availabilityClass(first):"mixed",popup=document.createElement("div");popup.className=`pin-popup ${isMultiple?"pin-popup-multiple":""}`;if(isMultiple){const heading=document.createElement("strong");heading.textContent=`この地点の店舗（${group.length}店）`;popup.append(heading)}group.forEach(store=>{const item=document.createElement("button"),title=document.createElement("strong"),address=document.createElement("span"),availability=document.createElement("span");item.type="button";item.className="pin-popup-store";title.textContent=store.name;address.textContent=store.address;availability.className=`popup-${availabilityClass(store)}`;availability.textContent=availabilityLabel(store);item.append(title,address,availability);item.addEventListener("click",()=>setSelectedId(store.id));popup.append(item)});const marker=L.marker([first.lat,first.lng],{icon:L.divIcon({className:"store-marker-shell",html:`<span class="store-marker ${isSelected?"selected":""} ${isMultiple?"multiple":""} availability-${markerStatus}">${isMultiple?group.length:""}</span>`,iconSize:[26,26],iconAnchor:[13,13],popupAnchor:[0,-13]}),bubblingMouseEvents:false}).bindPopup(popup,{closeButton:true,autoPan:true});marker.on("click",()=>{if(!isMultiple)setSelectedId(first.id)}).addTo(layer)})},[storeGroups,selectedId,mapReady]);
