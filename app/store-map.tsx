...
export default function StoreMap(){
  ...
  useEffect(()=>{const L=leafletRef.current?.default,layer=storeLayer.current;if(!L||!layer)return;layer.clearLayers();storeGroups.forEach(group=>{const first=group[0],isMultiple=group.length>1,isSelected=group.some(store=>store.id===selectedId),popup=document.createElement("div");popup.className=`pin-popup ${isMultiple?"pin-popup-multiple":""}`;if(isMultiple){const heading=document.createElement("strong");heading.textContent=`この地点の店舗（${group.length}店）`;popup.append(heading)}group.forEach(store=>{const item=document.createElement("button"),title=document.createElement("strong"),address=document.createElement("span");item.type="button";item.className="pin-popup-store";title.textContent=store.name;address.textContent=store.address;item.append(title,address);item.addEventListener("click",()=>setSelectedId(store.id));popup.append(item)});const marker=L.marker([first.lat,first.lng],{icon:L.divIcon({className:"store-marker-shell",html:`<span class="store-marker ${isSelected?"selected":""} ${isMultiple?"multiple":""}">${isMultiple?group.length:""}</span>`,iconSize:[26,26],iconAnchor:[13,13],popupAnchor:[0,-13]}),bubblingMouseEvents:false}).bindPopup(popup,{closeButton:true,autoPan:true});marker.on("click",()=>{if(!isMultiple)setSelectedId(first.id)}).addTo(layer)})},[storeGroups,selectedId,mapReady]);
